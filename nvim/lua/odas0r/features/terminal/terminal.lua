local Window = require("odas0r.features.terminal.window")
...
function Terminal:_spawn_shell(bufnr)
  ...
  local grp =
    api.nvim_create_augroup("TerminalBuffer_" .. bufnr, { clear = true })
...
function Terminal:close()
  ...
  if self.window:is_valid() then
    self.window:close()
    if
      current_winid == self.window.winid
      and self.last_winid
      and api.nvim_win_is_valid(self.last_winid)
    then
...
function Terminal:send(cmd)
  ...
  if job_id then
    ...
    vim.notify(
      "Terminal job not found. Please restart terminal.",
