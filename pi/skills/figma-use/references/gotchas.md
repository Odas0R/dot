...
> Part of the [figma_use skill](../SKILL.md). Every known pitfall with WRONG/CORRECT code examples.
...
The sync setter `figma.currentPage = page` does **NOT work** in `figma_use` — it throws `"Setting figma.currentPage is not supported"`. You **must** use `await figma.setCurrentPageAsync(page)` instead, which switches the page and loads its content.
...
// Then pass nodeId as a string literal in the next figma_use call
```
...
This applies to both `TextStyle` and `TextNode` properties. The same rule applies inside `figma_use`, interactive plugins, and any other plugin API context.
...
Sequential `import*ByKeyAsync` calls at the top of a `figma_use` script are a particularly common offender — design-system scripts often import a component set plus several variables plus an effect style in a row. **Always batch the imports:**
...
## combineAsVariants does NOT auto-layout in `figma_use`
