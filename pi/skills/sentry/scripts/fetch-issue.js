...
function formatEvent(event) {
		...
		for (const entry of event.entries) {
			if (entry.type === "request" && entry.data) {
				...
				if (req.headers && req.headers.length > 0) {
					...
					const headers = req.headers.filter(([k]) =>
						importantHeaders.includes(k),
					);
					if (headers.length > 0) {
						lines.push("");
				...
				if (crumbs.length > 0) {
					...
					for (const c of crumbs) {
						...
						if (!msg && c.data) {
							...
							else if (c.data.method)
								msg = `${c.data.method} ${c.data.url || ""}`;
	...
	if (!event.contexts && event.context) {
		...
		if (ctx.browser || ctx.os || ctx.device) {
			...
			if (ctx.os)
				lines.push(`- **OS:** ${ctx.os.name} ${ctx.os.version || ""}`);
			if (ctx.device)
				lines.push(`- **Device:** ${ctx.device.family || "unknown"}`);
		}
	}
...
async function main() {
	...
	if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
		...
		console.log(
			"  --org <org>  Organization slug (for short IDs like PROJECT-123)",
		);
		...
		console.log(
			"  fetch-issue.js https://sentry.io/organizations/sentry/issues/123/",
		);
