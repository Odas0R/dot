...
function formatBreadcrumb(crumb) {
	...
	if (!msg && crumb.data) {
		...
		else if (crumb.data.method)
			msg = `${crumb.data.method} ${crumb.data.url || ""}`;
...
function formatSpan(span, indent = 0) {
	...
	const duration = span.exclusive_time
		? `${span.exclusive_time.toFixed(1)}ms`
		: "?";
...
function formatEvent(event, options) {
	...
	lines.push("");
	lines.push(
		`**Timestamp:** ${formatTimestamp(event.dateCreated || event.timestamp)}`,
	);
		...
		for (const entry of event.entries) {
			if (entry.type === "exception" && entry.data?.values) {
				...
				for (const exc of entry.data.values) {
					lines.push("");
					lines.push(
						`**${exc.type || "Error"}:** ${exc.value || "(no message)"}`,
					);
